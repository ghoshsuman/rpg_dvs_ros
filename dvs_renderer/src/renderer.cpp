...
namespace dvs_renderer {
    ...
    image_tracking_(nh)
  {
    ...
    bool lsl_sync = true;
    if (lsl_sync) {
        std::thread lslThread(&Renderer::lsl_loop, this);
        lslThread.detach();
      }
  }
  ...
  void Renderer::lsl_loop() {
    lsl::stream_info info("ev_ts", "timestamp", 1, 1, lsl::cf_double64);
    lsl::stream_outlet outlet(info);
    double out_ts[1];

    std::cout << "inside lsl loop" << std::endl;

    // Waiting for at least one consumer to be connected
    //    while (!outlet.wait_for_consumers(1)) {
    //        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    //    }

    // Send data forever
    while (true) {
        // Assume that last_ts holds the timestamp value you want to send
        //        double last_ts = /*some value representing a timestamp*/;
//        static double t0 = latest_ts;
        out_ts[0] = (latest_ts).toSec();

        std::cout << "sending data" << out_ts[0] << std::endl;
        outlet.push_sample(out_ts);
