...
namespace dvxplorer_ros_driver {
  ...
  void DvxplorerRosDriver::callback(dvxplorer_ros_driver::DVXplorer_ROS_DriverConfig &config, uint32_t level) {
    ...
    if (level & (0x01 << 3)) {
        bkg_filter_enabled_ = config.background_noise_filter;
        hotpixel_filter_enabled_ = config.hot_pixel_filter;
      }
  }
    ...
    while (running_) {
        try {
          ...
          if(hotpixel_filter_enabled_) {
              if (!filter.configGet(CAER_FILTER_DVS_HOTPIXEL_LEARN)) {
                  filter.configSet(CAER_FILTER_DVS_HOTPIXEL_ENABLE, true);
                }
            }
          else {
              filter.configSet(CAER_FILTER_DVS_HOTPIXEL_ENABLE, false);
            }
